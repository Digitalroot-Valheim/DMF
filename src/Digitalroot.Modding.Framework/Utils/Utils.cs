...
namespace Digitalroot.Modding.Framework.Utils
{
  public static class Utils
  {
    ...
    public static class EnvironmentUtils
    {
      ...
      /// </summary>
      [UsedImplicitly]
      public static bool IsDedicated => ValheimUtils.IsDedicated;

      /// <summary>
      /// Does not work in Awake()
      /// </summary>
      [UsedImplicitly]
      public static bool IsServer => ValheimUtils.IsServer;

      /// <summary>
      /// Works in Awake()
      /// </summary>
      [UsedImplicitly]
      public static bool IsHeadless() => ValheimUtils.IsHeadless();

      // ReSharper disable once MemberCanBePrivate.Global
      public static string Namespace => $"{nameof(Digitalroot)}.{nameof(Modding)}.{nameof(Framework)}";
    }
    ...
    public static class ValheimUtils
    {
      ...
      public static bool IsObjectDBReady()
      {
        ...
        return (GetObjectDB() != null && GetObjectDB().m_items.Count != 0 && GetItemPrefab("Amber") != null) || EnvironmentUtils.IsRunningFromNUnit;
      }
      ...
      {
        Log.Trace(Logger, $"{EnvironmentUtils.Namespace}.{MethodBase.GetCurrentMethod()?.DeclaringType?.Name}.{MethodBase.GetCurrentMethod()?.Name}({prefabName}, {location}, {parent?.name})");
      ...
      {
        Log.Trace(Logger, $"{EnvironmentUtils.Namespace}.{MethodBase.GetCurrentMethod()?.DeclaringType?.Name}.{MethodBase.GetCurrentMethod()?.Name}({prefab.name}, {location}, {parent?.name})");
      ...
      /// <param name="resourceAssembly">Executing assembly</param>
      /// <returns></returns>
      public static AssetBundle LoadAssetBundleFromResources(string bundleName, Assembly resourceAssembly) => AssemblyUtils.LoadAssetBundleFromResources(bundleName, resourceAssembly);
    }
